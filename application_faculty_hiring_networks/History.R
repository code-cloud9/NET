...
mat2 <- as.matrix(read_csv("ready_to_use_data_matrices/History General Adjacency Matrix.csv",
                           col_names = FALSE))
colnames(mat2) <- NULL
mat2 <- unname(mat2)
Bs112_mat <- mat2
...
for (i in 1:replication){
  ...
  #'@_e_ij
  etas_debias <- incomplete_Us_standz(city_size, alpha=1.2, Bs112_mat)
  
  #'@-----------------eta3_estimates
  check_2[i] <- etas_debias$eta_est[2] 
  check_3[i] <- etas_debias$eta_est[3] 
  check_4[i] <- etas_debias$eta_est[4]
  check_5[i] <- etas_debias$eta_est[5]
}
