...
# col1: nondegentest$sigma_square_21
# col2: sqrt(log(city_size)/city_size)
# col3: nondegentest$t
# col4: studentuzed_t$test_stat[2]
...
ptime2 <- system.time({
  output_table <- foreach(i = 1:replication, .combine = rbind) %do% {   # use %dopar% if desired
    cat("Iteration:", i, "\n")
    #'@_e_ij
    #' null
    Error_term.example <- WE_null_normal(n = city_size)
    
    #' non-null
    # Error_term.example <- WE_alt_normal(n = city_size, C = sqrt(0.5))
    ...
    c(nondegentest$sigma_square_21,
...
print(paste(city_size, "-", "test eta2", "-", ptime2[3]))

write.csv(output_table, file = "a-null-200-Net1.2.csv", row.names = FALSE)
