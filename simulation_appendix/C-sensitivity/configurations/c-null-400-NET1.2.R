...
st_f_null_normal <- function(n){
  
  a_i.save <- rnorm(n, 1, 1) - 1
  b_j.save <- a_i.save
  ...
  ###
  e_ij.save <- matrix(rep(a_i.save, n), n, n) * matrix(rep(b_j.save, n), n, n, byrow = TRUE) + epsilon_ij.save
  diag(e_ij.save) <- NA
...
st_b_alt_normal <- function(n,C){
  ...
  ### Generate Error Terms
  ###
  e_ij.save <- C * matrix(rep(a_i.save, n), n, n) * matrix(rep(b_j.save, n), n, n, byrow = TRUE) +  epsilon_ij.save
  diag(e_ij.save) <- NA
...
eta5_nondegen_concentration_test <- function(n, error_mari){
  ...
  ###
  numerator_5 <- mean(c(a_3,b_4) * c(b_4,a_3)) - mean(xi.vec) * mean(xi.vec)  # cov(c(a_3,b_4), c(b_4,a_3))  # cov(a_3, b_4)
  # lam_est.save[11] <- lam_est.save[5]  # mean(a_3 * b_4) - 1
...
srm_test <- function(city_size,error_mari){
  ...
  # dataframe_net <- cbind(dataframe_net, data.frame(Group = rep(1,2450)))
  ...
  # p-values
  # pvals <- 2*(1-pnorm(abs(mod1$coef/mod1$se)))
...
# output table: 1000 rows, 4 columns
# col1: nondegentest_ets5$sigma_square_51
# col2: sqrt(log(city_size)/city_size)
# col3: nondegentest_ets5$t
# col4: studentuzed_t$test_stat[5]
output_table <- matrix(NA, replication, 4)
...
ptime5 <- system.time({
  output_table <- foreach(i = 1:replication, .combine = rbind) %dopar% {   # use %dopar% if desired
    cat("Iteration:", i, "\n")
    #'@_e_ij
    #' null
    Error_term.example <- st_f_null_normal(n = city_size)
    
    #' non-null
    # Error_term.example <- st_b_alt_normal(n = city_size, C = sqrt(0.5))
    ...
    c(nondegentest_ets5$sigma_square_51,
      sqrt(log(city_size) / city_size),
      nondegentest_ets5$t,
      studentuzed_t$test_stat[5]
    )
  }
})
...
write.csv(output_table, file = "c-null-400-Net1.2.csv", row.names = FALSE)
