...
for (city_size in city_sizes){
  ...
  r3 <- foreach(i=1:replication, .combine=rbind) %dopar% {
    ...
    #'@_Our_method_eta3
    studentuzed_t <- incomplete_Us_standz(city_size, alpha=1.4, Error_term.example$e_ij.save)
...
write.csv(test_stat,file='b-c0-normal-Net1.4.csv', row.names=FALSE)
