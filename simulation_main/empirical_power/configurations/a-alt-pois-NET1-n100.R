...
WE_alt_Pois <- function(n,C){
  
  ###
  ###'@Generate_Gamma_ij__Paras:a1,b1
  ###
  gamma_ij.save <- matrix(rpois(n^2, 1), n, n)
...
for (c_val in C_list){
  ...
  r2 <- foreach(i=1:replication, .combine=rbind) %dopar% {
    ...
    #'@_e_ij
    Error_term.example <- WE_alt_Pois(n = city_size, C = c_val)
...
write.csv(test_stat,file='a-alt-pois-Net1-n100.csv', row.names=FALSE)
# stopCluster(cl)  
