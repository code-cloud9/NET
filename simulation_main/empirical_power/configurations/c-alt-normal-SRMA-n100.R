...
for (c_val in C_list){
  ...
  r5 <- foreach(i=1:replication, .combine=rbind) %dopar% {
    ...
    #'@_e_ij
    Error_term.example <- st_d_alt_normal(n = city_size, C = c_val)
...
write.csv(test_stat,file='c-alt-normal-SRMA-n100.csv', row.names=FALSE)
