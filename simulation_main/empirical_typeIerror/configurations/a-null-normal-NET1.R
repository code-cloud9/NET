  ...
  ptime2 <- system.time({
    r2 <- foreach(i=1:replication, .combine=rbind) %dopar% {
      ...
      if(nondegentest$sigma_square_21 > sqrt(log(city_size)/city_size) ){
        ...
        studentuzed_t <- incomplete_Us_NO_debias_standz(city_size, alpha=1, Error_term.example$e_ij.save)
...
write.csv(test_stat,file='a-null-normal-Net1.csv', row.names=FALSE)
