...
st_d_null_pois <- function(n){
  
  a_i.save <- rpois(n, 1) - 1
  b_j.save <- a_i.save 
...
#'@_Simulation
city_sizes <- c(25,50,100,200,400)
replication <- 1000
...
for (city_size in city_sizes){
  ...
  ptime <- system.time({
    r <- foreach(i=1:replication, .combine=rbind) %dopar% {
      
      #'@_e_ij
      Error_term.example <- st_d_null_pois(n = city_size)
      
      
      #'@_SRM_ANOVA_method
      triplefit <- TripleR_test(city_size, Error_term.example$e_ij.save)
      triplefit[is.nan(triplefit)] <- 666  # track NA NaN out put 
      triplefit[is.na(triplefit)] <- 666  # track NA NaN out put 
      
      
      #'@_p-val_and_NA_of_test_eta3_and_eta5
      c(triplefit[1], triplefit[5])

    }
  })
    
  test_stat[,c(coln,coln+1)] <- r
  print(paste(city_size,"-", "test eta3/5","-", ptime[3]))

}

write.csv(test_stat,file='c-null-pois-SRMA.csv', row.names=FALSE)
