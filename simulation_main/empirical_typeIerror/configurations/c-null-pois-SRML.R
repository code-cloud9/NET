...
city_sizes <- c(25,50)
replication <- 1000
test_stat <- matrix(NA,replication,4)
...
for (city_size in city_sizes){
  ...
  ptime <- system.time({
    r <- foreach(i=1:replication, .combine=rbind) %dopar% {
      ...
      #'@_SRM_likelihood_method
      srmfit <- c(666,666)  # track NA NaN out put 
      try(
        invisible(capture.output(srmfit <- srm_test(city_size, Error_term.example$e_ij.save)))  
      )
...
write.csv(test_stat,file='c-null-pois-SRML.csv', row.names=FALSE)
